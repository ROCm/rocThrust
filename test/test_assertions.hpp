...
testing::AssertionResult ComplexCompare(const char* expr1,
  ...
  const thrust::complex<double> tol_diff(0.1 * (fabs(val1.real() + val2.real()) + abs_error.real()),
...
testing::AssertionResult ComplexNearPredFormat(const char* expr1,
  ...
  return ComplexCompare(expr1, expr2, abs_error_expr,
                        val1, val2, abs_error);
}
...
testing::AssertionResult ComplexVectorNearPredFormat(const char* expr1,
  ...
  {
    testing::AssertionResult result = ComplexCompare(expr1, expr2, abs_error_expr,
                                                     vector1[i], vector2[i], abs_error);
...
#define ASSERT_NEAR_COMPLEX_VECTOR_ERROR(val1, val2, abs_error)\
  ASSERT_PRED_FORMAT3(ComplexVectorNearPredFormat<typename decltype(val1)::value_type>, \
                      val1, val2, abs_error)


#define ASSERT_NEAR_COMPLEX_VECTOR(val1, val2)\
  ASSERT_NEAR_COMPLEX_VECTOR_ERROR(val1, val2, thrust::complex<T>(std::numeric_limits<T>::epsilon(),std::numeric_limits<T>::epsilon()))
