...
TEST(ScanTests, TestInclusiveScan32)
{
    ...
    thrust::host_vector<T> h_input = get_random_data<T>(n,
...
TYPED_TEST(ScanVariablesTests, TestScanWithOperatorToDiscardIterator)
{
    ...
    for(auto size : sizes) {
        ...
        h_result = thrust::exclusive_scan(h_input.begin(), h_input.end(), thrust::make_discard_iterator(), T(13),
                                       max_functor<T>());

        d_result = thrust::exclusive_scan(d_input.begin(), d_input.end(), thrust::make_discard_iterator(), T(13),
                                       max_functor<T>());
...
TEST(ScanTests, TestScanMixedTypes)
{
    ...
    thrust::host_vector<unsigned int> h_input = get_random_data<unsigned int>(n,
