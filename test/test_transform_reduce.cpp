...
TYPED_TEST(TransformReduceIntegerPrimitiveTests, TestTransformReduce)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      T gpu_result =
        thrust::transform_reduce(d_data.begin(), d_data.end(), thrust::negate<T>(), init, thrust::plus<T>());
...
TYPED_TEST(TransformReduceIntegerPrimitiveTests, TestTransformReduceFromConst)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      T gpu_result =
        thrust::transform_reduce(d_data.cbegin(), d_data.cend(), thrust::negate<T>(), init, thrust::plus<T>());
