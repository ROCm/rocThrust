...
TYPED_TEST(ReduceTests, TestReduceWithIndirection)
{
  ...
  T result =
    thrust::reduce(Policy{}, data.begin(), data.end(), T(0), plus_mod3<T>(thrust::raw_pointer_cast(&table[0])));
...
TYPED_TEST(ReducePrimitiveTests, TestReduceCountingIterator)
{
  ...
  for (auto size : sizes)
  {
    size_t n = std::is_integral<T>::value ? thrust::min<size_t>(size, std::numeric_limits<T>::max()) : size;
