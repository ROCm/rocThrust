...
TYPED_TEST(ZipIterator32BitTests, TestZipIteratorTransform)
{
  ...
  for (auto size : get_sizes())
  {
    ...
    for (auto seed : get_seeds())
    {
      ...
      thrust::host_vector<T> h_data1 = get_random_data<T>(size, 0, 10, seed + seed_value_addition);
      thrust::host_vector<T> h_data2 = get_random_data<T>(size, 0, 10, seed + 2 * seed_value_addition);
      ...
      transform(thrust::make_zip_iterator(thrust::make_tuple(h_data0.begin(), h_data1.begin())),
                thrust::make_zip_iterator(thrust::make_tuple(h_data0.end(), h_data1.end())),
                h_result.begin(),
                SumTwoTuple());
      transform(thrust::make_zip_iterator(thrust::make_tuple(d_data0.begin(), d_data1.begin())),
      ...
      transform(thrust::make_zip_iterator(thrust::make_tuple(h_data0.begin(), h_data1.begin(), h_data2.begin())),
                thrust::make_zip_iterator(thrust::make_tuple(h_data0.end(), h_data1.end(), h_data2.end())),
                h_result.begin(),
                SumThreeTuple());
      transform(thrust::make_zip_iterator(thrust::make_tuple(d_data0.begin(), d_data1.begin(), d_data2.begin())),
