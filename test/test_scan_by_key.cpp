...
TEST(ScanByKeyTests, TestScanByKeyLargeInput)
{
  ...
  for (auto seed : get_seeds())
  {
    ...
    thrust::host_vector<int> vals_sizes =
      get_random_data<int>(10, std::numeric_limits<int>::min(), std::numeric_limits<int>::max(), seed);

    thrust::host_vector<int> h_vals = get_random_data<int>(
      N, std::numeric_limits<int>::min(), std::numeric_limits<int>::max(), seed + seed_value_addition);
    thrust::device_vector<unsigned int> d_vals = h_vals;
