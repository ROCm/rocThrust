...
TYPED_TEST(MergeByKeyTests, MergeByKeySimple)
{
    ...
    const auto ends = thrust::merge_by_key(Policy{},
                                           a_key.begin(),
                                           a_key.end(),
                                           b_key.begin(),
                                           b_key.end(),
...
TYPED_TEST(PrimitiveMergeByKeyTests, MergeByKeyToDiscardIterator)
{
    ...
    for(auto size : get_sizes())
    {
        ...
        for(auto seed : get_seeds())
        {
            ...
            auto h_a_keys = get_random_data<T>(
                size,
            ...
            const discard_pair h_result = thrust::merge_by_key(h_a_keys.begin(),
                                                               h_a_keys.end(),
                                                               h_b_keys.begin(),
                                                               h_b_keys.end(),
                                                               h_a_vals.begin(),
            ...
            const discard_pair d_result = thrust::merge_by_key(d_a_keys.begin(),
                                                               d_a_keys.end(),
                                                               d_b_keys.begin(),
                                                               d_b_keys.end(),
                                                               d_a_vals.begin(),
