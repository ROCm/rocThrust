 ...
 */
   ...
   */
        __host__ __device__ complex(const complex<T>& z) = default;
#else
        /*! Default construct a complex number.
   */
   ...
   *  \param z The \p complex to copy from.
   */
        __host__ __device__ complex& operator=(const complex<T>& z) = default;
#else
        /*! Assign `z.real()` and `z.imag()` to the real and imaginary parts of this
   *  \p complex respectively.
   *
   ...
   */
    ...
    || THRUST_DEVICE_COMPILER == THRUST_DEVICE_COMPILER_HCC
        typedef typename detail::conditional<
            detail::is_same<T, float>::value,
            float2,
            typename detail::conditional<
                detail::is_same<T, float const>::value,
                float2 const,
                typename detail::conditional<
                    detail::is_same<T, double>::value,
                    double2,
                    typename detail::conditional<detail::is_same<T, double const>::value,
                                                 double2 const,
                                                 generic_storage_type>::type>::type>::type>::type
            storage_type;
#else
 ...
 */
    template <typename T0, typename T1>
    __host__ THRUST_STD_COMPLEX_DEVICE bool operator==(const complex<T0>&      x,
 ...
 */
    template <typename T0, typename T1>
    __host__ THRUST_STD_COMPLEX_DEVICE bool operator==(const std::complex<T0>& x,
                                                       const complex<T1>&      y);
