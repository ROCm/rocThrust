        ...
        namespace detail
        {
            namespace internal
            {
                ...
                public:
                    ...
                    __host__ __device__ index_range(index_type begin, index_type end)
                        : m_begin(begin)
                        , m_end(end)
                    {
                    }

                    __host__ __device__ index_type begin(void) const
                    {
                        return m_begin;
                    }

                    __host__ __device__ index_type end(void) const
                    {
                        return m_end;
                    }

                    __host__ __device__ index_type size(void) const
                    {
                ...
                public:
                    ...
                    __host__ __device__ uniform_decomposition(index_type N,
                                                              index_type granularity,
                                                              index_type max_intervals)
                        : m_N(N)
                    ...
                    __host__ __device__ index_range<index_type>
                                        operator[](const index_type& i) const
                    {
                        ...
                        {
                            index_type begin = m_large_interval * m_threshold
                                               + m_small_interval * (i - m_threshold);
                            index_type end
                                = (begin + m_small_interval < m_N) ? begin + m_small_interval : m_N;
                            return range_type(begin, end);
                        }
                    }
