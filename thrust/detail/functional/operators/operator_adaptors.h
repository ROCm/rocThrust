...
struct transparent_unary_operator
{
  ...
  template <typename Env>
  THRUST_HOST_DEVICE result_type<Env> eval(Env&& e) const THRUST_RETURNS(UnaryFunctor{}(thrust::get<0>(THRUST_FWD(e))))
};
...
struct transparent_binary_operator
{
  ...
  template <typename Env>
  THRUST_HOST_DEVICE result_type<Env> eval(Env&& e) const
